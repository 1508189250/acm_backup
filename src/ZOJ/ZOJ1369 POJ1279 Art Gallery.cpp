...
void cut(const POINT &lhs,const POINT &rhs){
  ...
  for(int i = 0; i < n_e_cnt; ++i){
    ...
    //故应该接着判断
      ...
      if(_less(crossProduct(lhs,rhs,p[(i-1)%n_e_cnt]),0.0)) {//原理同上
        q[n_t_cnt++] = intersectPoint(lhs,rhs,p[i],p[(i-1)%n_e_cnt]);
      }

      if(_less(crossProduct(lhs,rhs,p[i+1]),0.0)){//原理同上
...
double solve(){
  ...
  if (_less_or_equal(area(points,n_point_count),0.0))
